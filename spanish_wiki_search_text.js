...
db.serialize(function() {
	...
	db.each("SELECT id, english_name, url FROM cartas WHERE bash IS NULL AND url != 1 LIMIT 0,100", function(err, row) {
		...
		request.get({ url: row.url, timeout: 600000}, function(error, response, body) {
			...
			$('#WikiaArticle').each(function( index ) {
				...
				if(english_name != row.english_name){
					...
					stmt.run(spanish_name, card_description_me, card_description_pe, set_card[0].substring(1).replace(/\s+/g, ''), row.id);
